...
const haushaltsbuch = {
    ...
    // betrag_verarbeiten zuerst Format des    valides format    betrag.validieren(betrag) via regex ob true oder false.
    ...
    gesamtbilanz_ausgeben() {
        console.log(`Einnahmen ${(this.gesamtbilanz.get("einnahme") / 100).toFixed(2)} €\n`
            + `Ausgaben ${(this.gesamtbilanz.get("ausgabe") / 100).toFixed(2)} €\n`
            + `Bilanz: ${(this.gesamtbilanz.get("bilanz") / 100).toFixed(2)} €\n`
            + `Die Bilanz ist positiv: ${(this.gesamtbilanz.get("bilanz") / 100) >= 0}\n`
